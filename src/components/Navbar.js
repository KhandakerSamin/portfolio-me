          ...
          <div className="p-6 space-y-2 max-h-[60vh] overflow-y-auto">
            {navLinks.map((link, index) => (
              <Link
                ...
                href={link.href}
                onClick={(e) => handleSmoothScroll(e, link.href)}
                className="group relative flex items-center justify-center text-gray-300 hover:text-white transition-all duration-300 text-base font-medium py-4 px-5 rounded-xl hover:bg-linear-to-r hover:from-blue-500/10 hover:to-purple-500/10 border border-transparent hover:border-blue-500/30"
              >
                <span className="relative z-10">{link.name}</span>
              </Link>
            ))}
